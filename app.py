...
# --- FIX DEFINITIF POUR LES CHEMINS (STREAMLIT CLOUD) ---
# On determine la racine du projet et le dossier src
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
SRC_DIR = os.path.join(BASE_DIR, 'src')

# On force l'ajout de ces dossiers dans le systeme de recherche Python
if BASE_DIR not in sys.path:
    sys.path.insert(0, BASE_DIR)
if SRC_DIR not in sys.path:
    sys.path.insert(0, SRC_DIR)

# --- IMPORTS DES MODULES METIER ---
try:
    # Tentative d'import direct (Standard Cloud)
    ...
    # Backup via le package src (Standard Local)
...
# CSS pour un look industriel propre
st.markdown("""
    <style>
...
# --- 2. DEMAND PLANNING (IA) ---
    ...
    if uploaded_sales:
        ...
        st.write("Le modele Random Forest analyse vos tendances...")
    else:
        st.info("Mode Simulation : Entrez une valeur manuellement.")
        last_val = st.number_input("Dernieres ventes connues", value=1000)
...
# --- 5. DIGITAL TWIN (RESILIENCE) ---
    ...
    st.line_chart(levels, use_container_width=True)
    
    if min(levels) <= 0:
        st.error("RUPTURE DE STOCK DETECTEE. Action proactive requise.")
    else:
        st.success("Chaine logistique resiliente.")
